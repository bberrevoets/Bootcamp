...
public static class HttpResponseMessageExtensions
{
    ...
    {
        if (response.IsSuccessStatusCode) return new CommandResult(true);
        ...
        else if (!string.IsNullOrEmpty(problemDetails?.Title)) errors.Add(problemDetails.Title);

        if (problemDetails?.Extensions.TryGetValue("errors", out var value) == true &&
